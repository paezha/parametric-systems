...
Twin turtle spirals Panel 1:
```{r}
# Set random seed
seed_1 <- sample.int(10000, 1)
set.seed(seed_1)
...
panel_1 <- ggplot() +
  ...
  theme(legend.position = "none",
        ...
        plot.background = element_rect(color = NA, 
                                       fill = "white"),
        plot.margin = margin(0.25, 0.125, 0.25, 0.25, "cm"))

dx <- max(df$x) - min(df$x)
dy <- max(df$y) - min(df$y)
```

Twin turtle spirals Panel 2:
```{r}
# Set random seed
seed_2 <- sample.int(10000, 1)
set.seed(seed_2)
...
panel_2 <- ggplot() +
  ...
  theme(legend.position = "none",
        ...
        plot.background = element_rect(color = NA, 
                                       fill = "white"),
        plot.margin = margin(0.25, 0.0, 0.25, 0.125, "cm"))

dx <- dx + max(df$x) - min(df$x)
dy <- max(dy, max(df$y) - min(df$y))
```

Twin turtle spirals Panel 3:
```{r}
# Set random seed
seed_3 <- sample.int(100000, 1)
set.seed(seed_3)
...
panel_3 <- 
  ...
  theme(legend.position = "none",
        ...
        plot.background = element_rect(color = NA, 
                                       fill = "white"),
        plot.margin = margin(0.25, 0.25, 0.25, 0.125, "in"))
...
```{r}
if(dx > dy){
  ggsave(glue("twin-spiral-panel-{seed_1}-{seed_2}-{seed_3}.png"),
         width = 7)
}else{
  ggsave(glue("twin-spiral-panel-{seed_1}-{seed_2}-{seed_3}.png"),
         height = 7)
